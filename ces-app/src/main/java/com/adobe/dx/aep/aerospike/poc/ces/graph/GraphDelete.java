 ...
 *
 */
public abstract class GraphDelete extends GraphOperation {

    private final Writer writer;

    public GraphDelete(Writer writer, ThreadContext tc, GraphState s) {
        super(tc, s);
        this.writer = writer;
    }

    @SneakyThrows
    public Set<RecordValue> deleteEdge(Edge testEdge) {
        Set<RecordValue> graph = null;
        try {
            graph = writer.deleteEdge(testEdge.getEdge());
            // edge deleted
