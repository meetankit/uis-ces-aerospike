package com.adobe.dx.aep.aerospike;
...
/**
 *
 *  * aerospike multi record library class
 *
...
public class TransactionManager {
     ...
     */
    private final com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport client;
     ...
     */
    public TransactionManager(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport client){
        this.client = client;
    ...
    public int removeOrphanLocks(){
        ...
        txnStmt.setSetName(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TRANSACTION_SET);
        txnStmt.setBinNames(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TXN_ID_BIN_NAME);
        txnStmt.setFilter(Filter.equal(com.adobe.dx.aep.aerospike.Constants.TYPE_BIN_NAME, com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TXN_TYPE));
        ...
        while(txnRecords.hasNext()){
            txnHash.put(txnRecords.next().record.getString(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TXN_ID_BIN_NAME),1);
        }

        // Get all locks
        Statement stmt = new Statement();
        stmt.setNamespace(client.getTransactionNamespace());
        stmt.setSetName(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.LOCK_SET);
        stmt.setFilter(Filter.equal(com.adobe.dx.aep.aerospike.Constants.TYPE_BIN_NAME, com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.LOCK_TYPE));
        stmt.setFilter(Filter.range(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TIMESTAMP_BIN_NAME, 0,  System.currentTimeMillis() - transactionTimeOutMillis));

        // If the txn they are associated with does not exist remove them
        for (KeyRecord r : client.query(queryPolicy, stmt)) {
            String txnID = r.record.getString(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TXN_ID_BIN_NAME);
    ...
    private Vector<String> getExpiredTxnIDs(){
        ...
        txnStmt.setBinNames(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TXN_ID_BIN_NAME);
        txnStmt.setFilter(Filter.equal(com.adobe.dx.aep.aerospike.Constants.TYPE_BIN_NAME, com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TXN_TYPE));
        txnStmt.setFilter(Filter.range(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TIMESTAMP_BIN_NAME, 0, System.currentTimeMillis() - transactionTimeOutMillis));

        for (KeyRecord keyRecord : client.query(queryPolicy, txnStmt)) {
            txnIDList.addElement(keyRecord.record.getString(com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TXN_ID_BIN_NAME));
        }
        return txnIDList;
    }
    ...
    public void setup(){
        ...
        try {
            if (client.get(indexCreateCheckReadPolicy, new Key(client.getTransactionNamespace(), INDEX_CREATION_RECORD_SET, lockIndexCreatedKey)) == null) {
                IndexTask task = client.createIndex(indexCreateCheckReadPolicy, client.getTransactionNamespace(), com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.LOCK_SET, com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.LOCK_TYPE,
                        com.adobe.dx.aep.aerospike.Constants.TYPE_BIN_NAME, IndexType.STRING);
                task.waitTillComplete();
                client.put(writePolicy, new Key(client.getTransactionNamespace(), INDEX_CREATION_RECORD_SET, lockIndexCreatedKey),
                        new Bin(com.adobe.dx.aep.aerospike.Constants.TYPE_BIN_NAME, INDEX_CREATED_TYPE),new Bin(VALUE_BIN_NAME, com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.LOCK_TYPE));
            }
        }
        ...
        try {
            if (client.get(indexCreateCheckReadPolicy, new Key(client.getTransactionNamespace(), INDEX_CREATION_RECORD_SET, txnIndexCreatedKey)) == null) {
                IndexTask task = client.createIndex(indexCreateCheckReadPolicy, client.getTransactionNamespace(), com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TRANSACTION_SET, com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TXN_TYPE,
                        com.adobe.dx.aep.aerospike.Constants.TYPE_BIN_NAME, IndexType.STRING);
                task.waitTillComplete();
                client.put(writePolicy, new Key(client.getTransactionNamespace(), INDEX_CREATION_RECORD_SET, txnIndexCreatedKey),
                        new Bin(com.adobe.dx.aep.aerospike.Constants.TYPE_BIN_NAME, INDEX_CREATED_TYPE),new Bin(VALUE_BIN_NAME, com.adobe.dx.aep.aerospike.AerospikeClientWithTxnSupport.TXN_TYPE));
            }
        }
        catch(AerospikeException e){
            if(e.getResultCode() == ResultCode.INDEX_ALREADY_EXISTS){
