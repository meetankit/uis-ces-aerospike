...
public class AerospikeClientWithTxnSupport extends AerospikeClient implements TxnSupport {
     ...
     * @param transactionNamespace namespace to be used for lock and transaction records
     */
    public AerospikeClientWithTxnSupport(ClientPolicy clientPolicy, String transactionNamespace, Host... hosts) {
        super(clientPolicy, hosts);
        this.transactionNamespace = transactionNamespace;
    ...
    private void blockForTurn(ThreadContext threadCtx, String step) {
        if( threadCtx != null) {
     ...
     */
    @Override
    public final void put(WritePolicy writePolicy, HashMap<Key, Bin[]> recordsForUpdate,
                          HashMap<Key, Integer> generationCheckMap, String txnID)
            throws LockAcquireException, GenFailException {
        put(writePolicy, recordsForUpdate, generationCheckMap, txnID, null, null);
    }

    public final void put(WritePolicy writePolicy, HashMap<Key, Bin[]> recordsForUpdate,
                          HashMap<Key, Integer> generationCheckMap, String txnID, ThreadContext tc, GraphState state)
            throws LockAcquireException, GenFailException {
        ...
        try {
            ...
            while (txnKeys.hasNext()) {
                ...
                if (recordsForUpdate.get(key) != null) {
                    ...
                    if(!locked && tc!= null && key.userKey.toString().equals(tc.getRunContext().getDesc()+"-0"))  {
                        blockForTurn(tc, "create transaction " + txnID + " key" + key.userKey);
                        locked = true;
                    }
                }
                else {
                    if (isEnterprise) writePolicy.durableDelete = true;
                    else LOGGER.warning("Warning - non durable delete taking place as isEnterprise = false");
            ...
            if(state != null) {
                //mark the tx successful for read validation
                GraphState.updateStatus(tc, state);
            }
    ...
    public void createLock(Key key, String txnID) throws LockAcquireException {
        ...
        try {
            ...
            if (ae.getResultCode() == ResultCode.KEY_EXISTS_ERROR) {
                ...
                if (r == null || !r.getString(TXN_ID_BIN_NAME).equals(txnID))
                    throw new LockAcquireException(key, txnID);
            } else {
                throw new TxnException(txnID, ae);
            }
        }
    ...
    public void removeLock(Key key, String txnID) {
        Key lockKey = lockKey(key);
        Expression txnEqual = Exp.build(Exp.eq(Exp.stringBin(TXN_ID_BIN_NAME), Exp.val(txnID)));
        WritePolicy deleteLockPolicy = new WritePolicy(txnWritePolicy);
        deleteLockPolicy.filterExp = txnEqual;
        try {
     ...
     */
    public void removeLocksForKeys(Iterator<Key> keys, String txnID) {
        while (keys.hasNext()) {
            removeLock(keys.next(), txnID);
        }
    }

    public void createLocks(Iterator<Key> keys, String txnID) throws LockAcquireException {
        while (keys.hasNext()) {
            createLock(keys.next(), txnID);
        }
    }
    ...
    static byte[] stringToByteArray(String byteArrayAsString) {
        if (byteArrayAsString.length() % 2 != 0)
